...
exports.createQueue = function( client, pubsubClient ){
  return new Queue( client, pubsubClient );
};
...
function Queue( client, pubsubClient ) {
  this.client = client;
  ...
  setInterval(function(){
    client.sort('q:jobs:delayed'
      ...
      while (jobs.length) {
        ...
        // it as inactive.
        if (promote) {
          Job.get( self, id, function(err, job){
            if (err) return;
...
Queue.prototype.process = function(type, n, fn){
  ...
  while (n--) {
    ...
    })(new Worker(this, type, workerClient).start(fn));
  }
};
