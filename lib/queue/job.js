...
var search;
function getSearch( client ) {
    if (search) return search;
    reds.createClient = function() { return client; };
...
exports.get = function( queue, id, prefix, fn){
    var client = queue.client
        , job = new Job( queue, null, null, prefix );
...
function Job( queue, type, data, prefix, searchEnabled ) {
    this.type = type;
    this.data = data || {};
...
Job.prototype.log = function(str){
    ...
    this.client.rpush( this.prefix + ':job:' + this.id + ':log', str);
...
Job.prototype.set = function(key, val, fn){
    this.client.hset( this.prefix + ':job:' + this.id, key, val, fn || noop);
    return this;
};
...
Job.prototype.get = function(key, fn){
    this.client.hget( this.prefix + ':job:' + this.id, key, fn || noop);
    return this;
};
...
Job.prototype.progress = function(complete, total){
    ...
    this.set('progress', n);
    this.set('updated_at', Date.now());
    this.events.emit(this.id, 'progress', n);
    return this;
};
...
Job.prototype.remove = function(fn){
    ...
    if ( this.searchEnabled) {
        getSearch( this.client ).remove(this.id);
    }

    this.client.del( this.prefix +':job:' + this.id + ':log', noop);
    this.client.del( this.prefix +':job:' + this.id, fn || noop);
    return this;
};
...
Job.prototype.removeState = function(fn){
    var client = this.client
        , state = this._state;
    client.zrem( this.prefix + ':jobs', this.id);
    client.zrem( this.prefix + ':jobs:' + state, this.id);
    client.zrem( this.prefix + ':jobs:' + this.type + ':' + state, this.id);
    return this;
};
...
Job.prototype.state = function(state){
    ...
    client.zadd( this.prefix + ':jobs', this._priority, this.id);
    client.zadd( this.prefix + ':jobs:' + state, this._priority, this.id);
    client.zadd( this.prefix + ':jobs:' + this.type + ':' + state, this._priority, this.id);
    // increase available jobs, used by Worker#getJob()
    if ('inactive' == state) client.lpush( this.prefix + ':' + this.type + ':jobs', 1);
    this.set('updated_at', Date.now());
    return this;
};
...
Job.prototype.update = function(fn){
    ...
    // search data
    if ( this.searchEnabled) {
        getSearch( this.client ).index(json, this.id);
    }
};
