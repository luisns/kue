...
function Worker(queue, type, workerClient, prefix) {
    this.queue = queue;
    this.type = type;
...
Worker.prototype.failed = function(job, err, fn){
    var self = this;
    this.events.emit(job.id, 'failed');
...
Worker.prototype.process = function(job, fn){
    ...
    fn(job, function(err){
        ...
        self.events.emit(job.id, 'complete');
        self.start(fn);
    });
    return this;
};
...
Worker.prototype.getJob = function(fn){
    ...
    var client = this.workerClient;

    // BLPOP indicates we have a new inactive job to process
    client.blpop( this.prefix + ':' + self.type + ':jobs', 0, function(err) {
        self.zpop( self.prefix + ':jobs:' + self.type + ':inactive', function(err, id){
            if (err) return fn(err);
            if (!id) return fn();
            Job.get( self.queue, id, self.prefix, fn);
        });
    });
};
